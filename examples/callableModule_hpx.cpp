...
idp_t Test(idp_t rsc_idp)
{
    ...
    // auto valor = data->getIdpValue();
    // std::cout << "getIdpValue: " << valor << std::endl;

    // data->AcquireWrite();
    // auto value = data->Get();
    // auto valor = data->fetch();
    // std::cout << "data->fetch(): " << valor << std::endl;




    // **data = agent_idp;
    // std::cout << "VALOR do data1: " << **data << std::endl;


    // **data = agent_idp;


    // std::cout << "VALOR do data2: " << **data << std::endl;

    // std::cout << "VALOR do data: " << data->fetch() << std::endl;
    // *value = agent_idp;
    // data->ReleaseWrite();

    // // obter uma referência para o agente que está a executar a função
    // auto agent = domain->GetLocalResource<cor::Agent<idp_t(idp_t)>>(agent_idp);
