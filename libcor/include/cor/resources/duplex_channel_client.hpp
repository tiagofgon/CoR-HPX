#ifndef COR_DUPLEXCHANNEL_CLIENT_HPP
#define COR_DUPLEXCHANNEL_CLIENT_HPP

#include "cor/resources/duplex_channel.hpp"
...
template <typename T>
class DuplexChannel_Client : hpx::components::client_base<DuplexChannel_Client<T>, DuplexChannel<T>> 
{

public:
	typedef hpx::components::client_base<DuplexChannel_Client<T>, DuplexChannel<T>> base_type;
    ...
    DuplexChannel_Client()
    {}

	// Create a client side representation for the existing
	// DuplexChannel instance with the given GID
    DuplexChannel_Client(hpx::future<hpx::id_type> && id) :
        base_type(std::move(id)),
		_idp(IdpGlobal())
    {}

    DuplexChannel_Client(hpx::shared_future<hpx::id_type> && id) :
        base_type(std::move(id)),
		_idp(IdpGlobal())
    {}

    DuplexChannel_Client(hpx::id_type && id) :
        base_type(std::move(id)),
		_idp(IdpGlobal())
    {}

	// Constructor for replicas
    DuplexChannel_Client(idp_t idp, hpx::future<hpx::id_type> && id) :
        base_type(std::move(id)),
		_idp(idp)
    {}

    DuplexChannel_Client(idp_t idp, hpx::shared_future<hpx::id_type> && id) :
        base_type(std::move(id)),
		_idp(idp)
    {}

    DuplexChannel_Client(idp_t idp, hpx::id_type && id) :
        base_type(std::move(id)),
		_idp(idp)
    {}

	// Standard constructor with parameters
    DuplexChannel_Client(idp_t idp, std::string const& myself, std::string const& partner) :
        base_type(create_server(idp, myself, partner)),
		_idp(idp)
    {}

    DuplexChannel_Client(idp_t idp, hpx::id_type locality, std::string const& myself, std::string const& partner) :
        base_type(create_server_remote(idp, locality, myself, partner)),
		_idp(idp)
    {}
	...
	/** DuplexChannel's interface **/
    hpx::future<T> Get(std::size_t step) 
	{
		typedef typename DuplexChannel<T>::Get_action_DuplexChannel action_type;
		return action_type()(this->get_id(), step); 	
	}

    void Set(T&& t, std::size_t step) 
	{
		typedef typename DuplexChannel<T>::Set_action_DuplexChannel action_type;
...
private:
	hpx::future<hpx::id_type> create_server(idp_t idp, std::string const& myself, std::string const& partner) {
		return hpx::local_new<DuplexChannel<T>>(idp, myself, partner);
	}

	hpx::future<hpx::id_type> create_server_remote(idp_t idp, hpx::id_type locality, std::string const& myself, std::string const& partner) {
		return hpx::new_<DuplexChannel<T>>(locality, idp, myself, partner);
	}
