  ...
  public:
	...
	void registerGenericRPC()
	{
		...
		info->dispatcher = [this](Type& obj, Stream& in, InProcessorData& out, Transport& trp, std::string group, Header hdr) {
			...
			if (!info)
			{
				details::Send::error(trp, group, hdr, "Generic RPC not found");
				return;
			}

			info->dispatcher(obj, in, out, trp, group, hdr);
		};
		...
		info->dispatcher = [f](Type& obj, Stream& in, InProcessorData& out, Transport& trp, std::string group, Header hdr) {
			...
			if (hdr.isGenericRPC())
			{
				...
				if (!toTuple(a, params))
				{
					...
					details::Send::error(trp, group, hdr, "Invalid parameters for generic RPC");
					return;
				}
			}
			else
			...
			details::Dispatcher<Traits::isasync, R>::impl(obj, f, std::move(params), out, trp, group, hdr);
		};
		m_rpcs.push_back(std::move(info));
	}
	...
	void registerControlRPC(const char* name, F f)
	{
		...
		info->dispatcher = [f, info=info.get()](Type& obj, Stream& in, InProcessorData& out, Transport& trp, std::string group, Header hdr) {
			...
			details::Send::result(trp, group, hdr, o);
		};
